...
namespace Calendar
{
	public partial class FormAllSchedules : Form
	{
				...
				for (int i = 1; i < DateTime.DaysInMonth(Year, Month) + 1; i++ )
				{
					...
					switch( p[new DateTime(Year, Month, i)])
					{
						case global::StandbyList.PossibleDays.Status.Affair:
							row.Cells[i].Value = "×";
							break;
						case global::StandbyList.PossibleDays.Status.Duty:
							row.Cells[i].Value = "当";
							row.Cells[i].Style.ForeColor = Setting.DutyColor;
							break;
		...
		private void dgView_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			{
				case "×":
					dgView[e.ColumnIndex, e.RowIndex].Value = "当";
					dgView[e.ColumnIndex, e.RowIndex].Style.ForeColor = Setting.DutyColor;
					(dgView.Rows[e.RowIndex].Cells[0].Value as StandbyList.Person)[new DateTime(Year, Month, e.ColumnIndex)] = global::StandbyList.PossibleDays.Status.Duty;
					break;
				case "当":
